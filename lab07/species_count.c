...
void species_count(
    char species[],
    int n_sightings,
    struct pod sightings[n_sightings],
    int *n_pods,
    int *n_whales) {
    int i = 0;
    int whaleCount = 0;
    int podCount = 0;
    while (i < n_sightings) {
        struct pod sighting = sightings[i];
        if (strcmp(sighting.species, species) == 0) {
            whaleCount += sighting.how_many;
            podCount++;
        }
        i++;
    }
