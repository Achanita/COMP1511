...
int array_maximum(int size, int array[size]) {
    ...
    // Set the current maximum value to the first element in the array
    int max = array[0];

    int i = 1;
    while (i < size) {
        // if the current maximum is less than the current element in
        // the array, set the maximum to the current array element
        if (max < array[i]) {
            max = array[i];
        }
        i++;
