//
// COMP1511 lab10 - Padding (moving all elements) a character list
//
...
void pad_left(struct character_node *characters, char pad_character) {
    if (characters != NULL) {
        struct character_node *current = characters->next;
        char previous_char = characters->data;
        characters->data = pad_character;
        while (current != NULL) {
            char current_char = current->data;
            current->data = previous_char;
